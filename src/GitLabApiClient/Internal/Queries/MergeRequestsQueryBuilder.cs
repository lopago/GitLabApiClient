...
namespace GitLabApiClient.Internal.Queries
{
    internal class MergeRequestsQueryBuilder : QueryBuilder<MergeRequestsQueryOptions>
    {
        protected override void BuildCore(Query query, MergeRequestsQueryOptions options)
        {
            ...
            if (options.DeployedBefore.HasValue)
            {
                query.Add("deployed_before", options.DeployedBefore.Value);
            }

            if (options.DeployedAfter.HasValue)
            {
                query.Add("deployed_after", options.DeployedAfter.Value);
            }

            if (options.Wip.HasValue)
            {
                query.Add("wip", options.Wip.Value ? "yes" : "no");
            }
        }
